...
const Setup = async (req,res) => {
    const {email,username,bankDetails,displayName,galaPrice,bio,pin,profilePicture} = req.body
    try {
        const existingUser = await prisma.user.findUnique({where:{email}})

        if(!existingUser || (galaPrice < 500 || galaPrice > 5000)) return res.status(404).json("User Not Found or Invalid Gala Price")
